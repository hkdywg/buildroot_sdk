                            ...
                            enum drm_bridge_attach_flags flags)
{
    ...
    ret = drm_of_find_panel_or_bridge(bridge->of_node, 1, -1, &ser->panel, NULL);
    if(ret < 0 && ret != -ENODEV)
        return ret;
...
static enum drm_connector_status max96745_bridge_detect(struct drm_bridge *bridge)
{
    struct max96745_bridge *ser = to_max96745_bridge(bridge);
    struct max96745 *max96745 = ser->parent;
...
static int max96745_bridge_probe(struct platform_device  *pdev)
{
    ...
    ser = devm_kzalloc(dev, sizeof(struct max96745_bridge),  GFP_KERNEL);
    if(!ser)
        return -ENOMEM;
...
MODULE_DESCRIPTION("maxim max96745 DP bridge driver");
MODULE_LICENSE("GPL");
