...
def gen_entrylist(parts, entrylist):
    maxsize = 0
    partition_num = 0
    lba_index = 2048
    lba_size = 512
    ...
    for p in parts:
        ...
        tmp = ((p["part_size"] + lba_size -1) / lba_size)
        name = p["label"]
        entry = create_gpt_entry(int(start), int(end), name.encode())
        entrylist.append(entry)
        maxsize = end
        lba_index = end + 1
        partition_num = partition_num + 1
...
if __name__ == '__main__':
    ...
    entrylist = []
    (alternate_lba, back_lba) = gen_entrylist(parts, entrylist)
